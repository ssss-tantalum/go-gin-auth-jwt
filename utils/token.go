...
func CreateToken(userId string, ttl time.Duration, privateKey string) (*TokenDetail, error) {
	now := time.Now().Local()
	td := &TokenDetail{
		ExpiresIn: new(int64),
		Token:     new(string),
	...
	*td.ExpiresIn = now.Add(ttl).Unix()
	...
	decotedPrivateKey, err := base64.StdEncoding.DecodeString(privateKey)
	if err != nil {
		return nil, fmt.Errorf("could not decode token private key: %w", err)
	}
	key, err := jwt.ParseRSAPrivateKeyFromPEM(decotedPrivateKey)
	if err != nil {
...
func ValidateToken(token string, publicKey string) (*TokenDetail, error) {
